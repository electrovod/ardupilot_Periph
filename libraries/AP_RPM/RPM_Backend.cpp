...
{
    const uint8_t esc_index;
#if 0
    esc_index = ap_rpm._params[state.instance].esc_telem_outbound_index;
    if (esc_index == 0) {
...
#endif // 0
    esc_index = 0;
    AP::esc_telem().update_rpm(esc_index-1, /* state.rate_rpm */ (AP_HAL::millis() & 0xFFF) *1.0 , 0);
}
#endif
